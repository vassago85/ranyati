...
class StoreAgreementRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $isEstate = $type === StorageAgreement::TYPE_DECEASED_ESTATE;
        $isSelf   = $type === StorageAgreement::TYPE_SELF_STORAGE;
        ...
        $validator->after(function (Validator $v) {
            ...
            foreach ($items as $i => $item) {
                ...
                if ($bookId) {
                    ...
                    if ($book && $book->type !== $this->input('type')) {
                        $v->errors()->add(
                            "items.$i.register_book_id",
                            'Selected register book ('.$book->code.') is for '.$book->type.', not the current '.$this->input('type').' intake.'
                        );
                    }
